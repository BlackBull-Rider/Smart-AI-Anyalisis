...
Module: backend/data/corporate_actions.py
Description: Enterprise Corporate Actions Synchronization Engine.
             Exclusive database write orchestration engine for corporate actions
             (Dividends, Splits, Bonus, Rights, Mergers, Demergers, etc.).
...
# EXCEPTIONS
# =========================================================================
class SyncError(Exception): """Base exception for Corporate Action Sync."""
...
# =========================================================================
class CorporateActionMetrics:
    _lock = threading.Lock()
    _stats: Dict[str, float] = defaultdict(float)

    @staticmethod
    def increment(name: str, amount: float = 1.0) -> None:
        with CorporateActionMetrics._lock: 
            CorporateActionMetrics._stats[f"corp_actions.{name}"] += amount

    @staticmethod
    def record_latency(name: str, duration_ms: float) -> None:
        with CorporateActionMetrics._lock:
            k = f"corp_actions.{name}_avg_ms"
            CorporateActionMetrics._stats[k] = (CorporateActionMetrics._stats[k] * 0.95) + (duration_ms * 0.05)

    @staticmethod
    def get_metrics() -> Dict[str, float]:
        with CorporateActionMetrics._lock: 
            return dict(CorporateActionMetrics._stats)
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                CorporateActionMetrics.record_latency(f"{component}_{operation}", dur_ms)
...
class StructuredLogger:
    ...
    def _log(self, level: int, msg: str, **kwargs):
        payload = {
            ...
            "component": "corporate_actions_sync",
...
_logger = StructuredLogger("CorporateActionsSync")
        ...
        while retries <= self.config.retry_count:
            try:
                ...
                if retries <= self.config.retry_count:
                    ...
                    CorporateActionMetrics.increment("retry_count")
                    _logger.warning("Deadlock detected. Retrying.", operation=func.__name__, retry=retries, delay=delay)
                    time.sleep(delay)
        
        CorporateActionMetrics.increment("deadlock_failures")
...
class SQLiteCorporateActionAdapter(CorporateActionAdapter):
    def __init__(self, config: CorporateActionSyncConfig):
        self.config = config
...
class PostgresCorporateActionAdapter(CorporateActionAdapter):
    def __init__(self, config: CorporateActionSyncConfig):
        self.config = config
...
class TransactionManager:
    ...
    def __init__(self, adapter: CorporateActionAdapter):
    ...
    def begin_transaction(self) -> None:
        ...
        try:
            if isinstance(self.adapter, SQLiteCorporateActionAdapter):
                conn.execute("BEGIN IMMEDIATE")
            else:
                conn.autocommit = False
            self._local.in_transaction = True
            CorporateActionMetrics.increment("transactions")
        except Exception as e:
    ...
    def rollback(self) -> None:
        ...
        try:
            ...
            CorporateActionMetrics.increment("rollback_count")
        except Exception as e:
...
# =========================================================================
# ENTERPRISE CENTRAL CORPORATE ACTIONS SYNC
# =========================================================================
class CorporateActionSync:
    """
    Central Synchronization Layer for Corporate Actions Data.
    Handles bulk writes, Hash-based Idempotency, Cache Invalidations, and atomic syncs.
    Strictly isolated from business logic.
    """

    FALLBACK_VALID_TABLES: Set[str] = {
        "corporate_actions", "dividends", "stock_splits", "bonus_issues",
        "rights_issues", "buybacks", "mergers", "demergers", "spin_offs",
        "delistings", "face_value_changes", "symbol_changes", "board_meetings"
    }

    def __init__(self, config: CorporateActionSyncConfig = CorporateActionSyncConfig()):
        self.config = config
        self.adapter = SQLiteCorporateActionAdapter(config) if config.dialect == Dialect.SQLITE else PostgresCorporateActionAdapter(config)
        self.tx_manager = TransactionManager(self.adapter)
        self.executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=config.async_workers, 
            thread_name_prefix="CorpActSyncAsync"
        )
        try:
            self.valid_tables = set(self.adapter.get_tables())
            if not self.valid_tables:
                self.valid_tables = self.FALLBACK_VALID_TABLES
        except Exception as e:
            _logger.warning("Failed to initialize dynamic valid tables list. Fallback required.", error=str(e))
            self.valid_tables = self.FALLBACK_VALID_TABLES
            
        _logger.info("CorporateActionSync Engine initialized.", dialect=config.dialect.value)
    ...
    def _inject_metadata(self, table_name: str, records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        has_created = "created_at" in db_cols
        has_updated = "updated_at" in db_cols

        for r in records:
            if has_created and "created_at" not in r:
                r["created_at"] = sync_timestamp
            if has_updated:
                r["updated_at"] = sync_timestamp
            ...
            if self.config.enable_hash_comparison and has_hash:
                raw_str = json.dumps({k: v for k, v in r.items() if k not in ("record_hash", "version_timestamp", "updated_at")}, sort_keys=True, default=str).encode('utf-8')
                r["record_hash"] = hashlib.sha256(raw_str).hexdigest()
                
        return records

    def _invalidate_cache(self, table_name: str) -> None:
        CorporateActionMetrics.increment("cache_invalidation")
    ...
    def _execute_write(self, table_name: str, records: List[Dict[str, Any]], mode: WriteMode) -> int:
        ...
        if mode in (WriteMode.UPSERT, WriteMode.REPLACE):
        ...
        with self.tx_manager.atomic():
            for i in range(0, len(tuples_data), self.config.chunk_size):
                ...
                total += len(chunk)
                CorporateActionMetrics.increment("rows_written", len(chunk))
                if mode in (WriteMode.UPSERT, WriteMode.REPLACE) and self.config.enable_hash_comparison and "record_hash" in target_cols:
                    if rowcount < len(chunk):
                        CorporateActionMetrics.increment("rows_skipped", len(chunk) - rowcount)
    ...
    def update(self, table_name: str, update_data: Dict[str, Any], conditions: Dict[str, Any]) -> int:
        ...
        CorporateActionMetrics.increment("rows_updated", rowcount)
        if self.config.enable_audit_logging: 
            AuditEngine.record_write(t, AuditAction.UPDATE, rowcount, (time.perf_counter() - t0)*1000)
    ...
    def delete(self, table_name: str, conditions: Dict[str, Any]) -> int:
        ...
        if self.config.enable_audit_logging: 
            AuditEngine.record_write(t, AuditAction.DELETE, rowcount, (time.perf_counter() - t0)*1000)
        self._invalidate_cache(t)
        return rowcount
    ...
    def sync_dataframe(self, table_name: str, df: Any) -> int:
        if not PANDAS_AVAILABLE: raise ConfigurationError("Pandas is required.")
    ...
    def sync_numpy(self, table_name: str, array: Any, columns: List[str]) -> int:
        if not PANDAS_AVAILABLE: raise ConfigurationError("Pandas is required.")
    ...
    def statistics(self) -> Dict[str, Any]:
        return {
            "telemetry": CorporateActionMetrics.get_metrics(),
...
CORPORATE_ACTION_SYNC_REGISTRY["sync_pipeline"] = CorporateActionSync.sync_pipeline
CORPORATE_ACTION_SYNC_REGISTRY["sync_symbol"] = CorporateActionSync.sync_symbol
CORPORATE_ACTION_SYNC_REGISTRY["sync_incremental"] = CorporateActionSync.sync_incremental
...
__all__ = [
    "CorporateActionSync",
    "CorporateActionSyncConfig",
    "SQLiteCorporateActionAdapter",
    "PostgresCorporateActionAdapter",
    "TransactionManager",
    "CorporateActionMetrics",
    "register_sync",
    "Dialect",
    "WriteMode",
    "SyncError",
