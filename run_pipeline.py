...
TEST_STOCKS = [
    "ADANIPOWER", "ADANIENT", "ADANIPORTS", "APOLLOHOSP",
    "ASHOKLEY", "ASIANPAINT", "AXISBANK", "BAJAJ-AUTO",
    "BAJAJCON", "BAJAJFINSV", "BAJFINANCE", "BALAMINES",
    "BANDHANBNK", "BANKBARODA", "BHARTIARTL", "BPCL",
    "BRITANNIA", "CANBK", "CENTRALBK", "CHOLAFIN",
    "CIPLA", "COALINDIA", "CPSEETF", "DCBBANK",
    "DIVISLAB", "DRREDDY", "EICHERMOT", "ETERNAL",
    "FEDERALBNK", "SEDEMAC", "TRENT"
]
...
def run():
    ...
    for symbol in TEST_STOCKS:
        ...
        try:
            ...
            if "trend" in trend:
                trend_state = trend["trend"].get("state", trend["trend"].get("regime", "Unknown"))
            else:
            ...
            print("Running Support & Resistance Analyzer...")
            sr = SupportResistanceAnalyzer().analyze(df)
            sr_state = sr.get("summary", {}).get("action", "WAIT")
            print("S&R :", sr_state)

            print("Running Smart Money Analyzer...")
            smc = SmartMoneyAnalyzer().analyze(df)
            smc_state = smc.get("summary", {}).get("action", "WAIT")
            print("Smart Money :", smc_state)
