...
def calculate_and_save(symbol):
    try:
        # ২. OHLCV ডেটা আনা হচ্ছে history টেবিল থেকে (stock_master থেকে নয়)
        query = f"SELECT * FROM history WHERE symbol='{symbol}' ORDER BY date ASC"
        raw_data = db.fetchall(query)
        
        if not raw_data:
            return symbol, False, 0, "No OHLCV data found in 'history' table"
...
if __name__ == "__main__":
    print(f"\n{'='*80}")
    print(" 🎯 LAYER-1B: SYMBOLS FROM [stock_master] -> DATA FROM [history] -> FEATURES")
    print(f"{'='*80}\n")
    
    try:
        # ১. stock_master থেকে শুধু এক্টিভ সিম্বলগুলো তোলা হচ্ছে
        try:
            rows = db.fetchall("SELECT symbol FROM stock_master WHERE is_active=1")
            if not rows: # Fallback in case is_active is not set
